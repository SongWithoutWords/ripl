...
Although small, this example illustrates a number of defining characteristics of the language:
...
2. ~ripl~'s syntax is Lisp-like, and consequently:
   1. Expressions can be grouped by parenthesis.
   2. Function application is performed by grouping (i.e. ~(f x1 ... xn)~), as in Haskell, ML and Lisp, as opposed to the more traditional mathematical notation (i.e. ~f(x1, ... xn)~) of languages with C-style syntax.
   3. There is neither operator precedence nor infix notation, which is elegant because the structure of the source code corresponds to the structure of the abstract syntax tree, but counter-intuitive because we're accustomed to reading mathematical expressions with conventional infix notation, operator precedence and associativity.
3. ~ripl~'s syntax includes some extensions over traditional Lisp syntax, inspired by [[https://sourceforge.net/p/readable/wiki/Home/][Readable Lisp S-expressions]]:
   ...
   2. Lines can be continued via indentation.
