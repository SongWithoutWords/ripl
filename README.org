...
*** TODO =?= Deterministic Destructor Calls (RAII)
...
[fn:26] ~ripl~ began without subtyping or overloading and used constraint generation and the unification algorithm for type inference. When I added overloading to ~ripl~, I continued to use the unification algorithm by deferring the unification of constraints with overloads until the types of the overloads and the types of the arguments were known, which worked. When I then added subtyping to the language it became clear that unification would no longer work, and I reverted to bidirectional type checking.
...
#+BEGIN_SRC racket
;; the union keyword can be used to create type-safe discriminated union
union expression
  struct add (expression a) (expression b)
  struct sub (expression a) (expression b)
...
*** TODO =+= Infix Notation, Selection Syntax, and Word Order

Most object-oriented languages allow code to be written in subject-verb-object (SVO) order (e.g. =map.contains(key)=) using methods, a word order that is used by 42% of the human population[fn:34], including speakers of English, French, Mandarin, and Spanish. By contrast, verb-subject-object (VSO) and verb-object-subject (VOS) order, which are used in traditional function application (e.g. =contains map key= or =contains key map=), are used by only 12% of the human population, including speakers of Arabic and Hebrew. The most popular word order, used by 45% of the world's population[fn:34], is subject-object-verb (SOV), but this order (as used by Forth and PostScript) is quite rare among programming languages.

Another aspect of programming languages and math that people are accustomed to is infix notation, which, like the method syntax of object-oriented languages, allows some or all verbs to appear in infix position, as in SVO ordering. Altogether this raises a few questions about the value of these syntactic features:

1. *Is SVO ordering of source code more intuitive for speakers of SVO languages*
2. *Is infix notation with precedence more intuitive in a mathematical context*
3. *Is there some inherent value to SVO ordering*



 Although I find it more convenient to add terms by writing =(+ x y z)= (as opposed to =(x + y + z)=), or to see if terms are ordered a certain way by writing =(< x y z)= (as opposed to =(x < y && y < z)=), programming languages with this ordering are famously unintuitive
 # [fn:35].

instead of the verb-subject-object (VSO) or verb-object-subject (VOS) order of typical function application (e.g. =contains map key= or =contains key map=).

Word orders in which the verb comes first (VSO and VOS) are quite uncommon, and are believed to be spoken by only 12% of the human population[fn:34]. By contrast, languages with SVO order are spoken by 42% of the human population[fn:34], and include English, French, Mandarin, and Spanish. 

[fn:34] Russell S. Tomlin, Basic word order. Functional principles. London: Croom Helm, 1986. Pp. 308, \newline
...
Suggesting SVO is the most intuitive word order, even among speakers of SOV
...
Overloading is a feature that is more likely to be noticed when absent than present. For example, in Haskell, which lacks overloading, you cannot have two functions with the same name, including simple accessors like =name= or =size=, without creating a type class that defines this function, and then implementing this type class for all types for which this function should be defined. As a result, when people use Haskell's record syntax, which automatically generates accessor functions, they often include the name of the type in the field name to avoid name-collisions, which is needlessly verbose.

Not only is this clumsy, but it results in some inconveniences when using Haskell's standard prelude and libraries:
1. The standard prelude defines =id= as the identity function, so the name =id= cannot be used for variables representing IDs.
2. The standard prelude defines =map= on lists, so the more general operation of mapping over functors (including lists) needed to be called =fmap=.
3. Because many collections, such as =Map= and =Set=, have operations with the same name, such as =size=, =null= and =empty=, it's usually necessary to import all of these modules qualified to avoid collisions, in which case they need to be referred to by a qualified name, such as =Set.size mySet= or =Map.size myMap=.
4. Because the =Num= typeclass defines the following interface: =(+) :: Num a => a -> a -> a=, =(-) :: Num a => a -> a=, =(*) :: Num a => a -> a -> a=, and =(/) :: Num a => a -> a -> a=, use of the standard prelude forces all types to implement the =Num= class in order to use these symbols, including vectors for which this interface makes no sense (the typical mathematical notion of vector multiplication and division by scalars does not satisfy this interface).

As much as I've heard overloading knocked (~TODO find examples~), the absence of overloading, as evidenced by Haskell, is quite inconvenient (so much so that there are a number of proposals within the Haskell community to fix this[fn:35]). In order to avoid these same inconveniences, ~ripl~ supports overloading, according to the following rules:
1. When an overloaded name is type checked, all definitions to which this name refers are considered.
2. The expression is type checked with each potential overload.
3. The overloads are then sorted in ascending order, first by the number of errors, and then by the number of warnings they produce within this context.
4. The first (best) overload is chosen.
5. If there is a tie for first place, then the overload is ambiguous and an error is raised.
...
It's not uncommon in my experience that people (myself included) conceive of and solve problems at a lower level of generality and abstraction than possible. When this occurs in programming language design, I think it introduces unnecessary complexity to the language. Some examples of this include:
...
# Templates are like generics/regular parametric polymorphism, except that they can also take integral numeric parameters in C++ and any object in D and ~ripl~. ~TODO: Maybe provide an example of people using type-level peano numbers in Haskell~
