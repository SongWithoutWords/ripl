...
Subtyping is a common feature among object oriented programming languages, and much less common among functional languages, possibly because of the complexity it adds to type inference, as discussed in section  [[=+= Type Inference]]. Although it is less essential in a language without inheritance (like ~ripl~), subtyping helps to reduce the need for explicit type conversions.

Although subtyping and implicit conversion have somewhat of a bad reputation (especially in languages like C in which they are rampant), I think that implicit conversions can still add value if chosen judiciously. Subtyping in ~ripl~ is achieved via implicit conversions. The ~ripl~ compiler provides a built-in conversions from integral numbers to floating point numbers, and may in future provide implicit conversions from the variants of a union to the union itself. Additionally, the compiler is structured to allow for user defined implicit conversions. Although the syntax for declaring user defined conversions has not yet been chosen, it will probably consist of defining a pure with a single input using a distinct keyword like ~implicit~ instead of the usual ~define~ keyword. An example of subtyping in ~ripl~ is given below:
...
# Essential and accidental complexity was coined by Brooks
# https://en.wikipedia.org/wiki/No_Silver_Bullet

Writing in 1987, Fred Brooks It has been suggested that there are two kinds of complexity that affect software development: the essential complexity of the problem at hand and the accidental complexity of the language, the tools, and the design of the program[fn:32]. 

I think there are two kinds of complexity that affect programming language designs. There is the inherent complexity of the problems the language is trying to solve, such as the complexity of data-races and memory management without garbage collection, which Rust solves with its ownership system. Then there is the accidental complexity of (in my experience) nearly everything in C++.
A large part of designing a language that can be easily understood (I believe) is to avoid complicated and duplicate features wherever possible.

A large part of maintaining a language design that can be easily understood (I believe) is to avoid complicated and duplicated features. Although 

This may be easiest to demonstrate by comparison with C++:
| Feature           | C++                                                                | ~ripl~          |
|-------------------+--------------------------------------------------------------------+-----------------|
| Code organization | Headers, namespaces and modules (expected in C++20[fn:31])         | Namespaces      |
| Enumerations      | ~enum~ and ~enum class~                                            | ~union~         |
| Looping           | ~for~, range-based ~for~, ~while~, ~do...while~, and recursion | Recursion       |
...
| Initialization    | ~A a(b)~, ~A a{b}~ and ~A a = A(b)~                                | ~define a(A b)~ |
...
[fn:32] Frederick Brooks, No Silver Bullet - Essence and Accident in Software Engineering,
