...
*** Hierarchical Clustering of Languages by Language Features
    ...
    The result of this clustering helps to classify ripl relative to other programming languages: it appears alongside other statically-typed, functional languages, after which it groups more closely with Rust than with ML or Haskell.

    The results show a high-level split between statically-typed, functional languages and the other languages considered. Within this statically-typed and functional branch, there is a split between languages with imperative capabilities (Rust and ripl), and the more purely functional languages (Haskell and ML). 
