...
Unfortunately Haskell solves the problems posed by side-effects and mutability by confining them to specific monads (namely IO and ST). Although this achieves the aim of differentiating between pure and impure computations, and does so in an elegant way, the combined abstractions of lazy evaluation, implicit indirection and monads make it much harder to reason about low-level performance, stateful computations, and highly interactive programs in Haskell than it is in many other languages, especially languages that are designed for this purpose like C++ and Rust. Maybe this is one of the reasons that Simon Peyton Jones has suggested that a future version of Haskell may be strictly evaluated, though still pure. ~TODO add source~
...
- Monads have a variety of uses beyoond modelling stateful computations
