...
2. It cuts down on visual tokens and visual clutter.
3. It ends any discussion or inconsistency over whether opening braces should occur on a new line.
4. It forces a one-to-one correspondence between the visual structure of the code (the indentation) and the actual structure of the code (the grouping by the parser), thereby reducing the potential for error and confusion.

For these reasons, expressions in ~ripl~ may be grouped by indentation.
