...
Overloading is a feature that is more noticeable when absent than present. In Haskell, for example, (which lacks overloading) you cannot have two functions with the same name (including simple accessors like =name= or =size=) without creating a type class that defines this function, and then implementing this type class for all required types. Sometimes people use type classes to overload functions, and at other times people include the name of the argument type in the name of the function or record field to avoid name collisions. This absence of overloading is clumsy, and has made Haskell's standard prelude inconvenient to use in a number of ways:
1. The standard prelude defines =id= as the identity function, so the name =id= cannot be used for variables that represents numeric identifiers (a common convention).
...
3. Because many collections, such as =Map= and =Set=, have operations with the same name, such as =size=, =null= and =empty=, it's often necessary to import modules qualified to avoid collisions, in which case their contents are referred to by a qualified name, such as =Set.size mySet= or =Map.size myMap=.
4. Because the =Num= type class defines the following interface: =(+) :: Num a => a -> a -> a=, =(-) :: Num a => a -> a=, =(*) :: Num a => a -> a -> a=, and =(/) :: Num a => a -> a -> a=, use of the standard prelude forces all types to implement the =Num= class in order to use these symbols, including vectors for which this interface makes no sense (the typical mathematical notion of vector multiplication and division by scalars does not satisfy this interface).

You can get around the problems mentioned above in Haskell by building your own prelude or using someone elses, but the fact remains that these inconveniences are a symptom of the absence of overloading.

As much as I've heard overloading knocked (~TODO find examples~), the absence of overloading is quite inconvenient, as evidenced by Haskell, (so much so that there are a number of proposals within the Haskell community to fix this[fn:35]). In order to avoid these same inconveniences, ~ripl~ supports overloading, according to the following rules:
