...
** Minimal Examples
~ripl~'s syntax is expression oriented (like Haskell, Lisp, Rust and Scala, among others) in that it does not distinguish between expressions and statements (as do C, C++, C#, Java, JavaScript, Python and others): most syntactic constructs in ~ripl~ are expressions that yield a value of some kind. More specifically, ~ripl~'s syntax is Lisp-like and inspired by [[https://sourceforge.net/p/readable/wiki/Home/][Readable Lisp S-expressions]]. Expressions are grouped by parenthesis, or by whitespace according to the following rules:
