...
[fn:32] This is especially true in languages like C, in which widespread and questionable implicit conversions allows some types to be used almost interchangeably that should not be, like integers, booleans and pointers.
...
Name overloading is not the kind of thing you notice when you have it.
Absence of name overloading is really annoying.
Add examples about Haskell's ~fmap~ vs ~map~, ~return~ vs ~pure~, and how much it would suck to add a vector class because ~+~ is a function of Num and Num doesn't have the right signatures for vector math.
