...
Unfortunately Haskell solves the problems posed by side-effects and mutability by confining them to specific monads (namely IO and ST). Although this achieves the aim of differentiating between pure and impure computations, and does so in an elegant way, the combined layers of abstraction of laziness, implicit indirection and monads make it much harder to reason about performance, stateful computations, and highly interactive programs in Haskell than it is in most other languages, especially C++ and Rust which shine in this regard.
