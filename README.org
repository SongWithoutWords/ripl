...
Although alternative algorithms have been implemented to support Hindley-Milner style type inference with subtyping[fn:27], ~ripl~ is proceding with bidirectional type inference for the following reasons:
...
2. Because the combination of all of ~ripl~'s intended type level features: overloading, subtyping, type classes, compile time evaluation, and templates that take both values and types, probably constitute too big a departure from the Hindley-Milner type system to allow for Hindley-Milner style type inference.
