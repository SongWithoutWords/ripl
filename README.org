...
Most object-oriented languages allow code to be written in subject-verb-object (SVO) order (e.g. =person.open(door)= or =map.contains(key)=) using methods, instead of the verb-subject-object (VSO) order of typical function application (e.g. =open person door= or =contains map key=).
...
Although I find it pretty intuitive to add some terms by writing =(+ x y z)=, or to see if terms are ordered a certain way by writing =(< min mid max)=, programming languages with this ordering are renowned for being unintuitive[fn:35].

Another aspect of programming languages and math that people are accustomed to is infix notation.
...
3. Needless distinction between non-virtual member methods and regular functions (solved either by universal function call syntax, as in D, or by not having methods, as in Haskell, ML, ~ripl~, Rust).
4. Needless distinction between array access notation and function application.
5. Needless distinction between functions and operators (solved).
