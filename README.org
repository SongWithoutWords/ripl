...
2. It is easily and intuitively encoded in function types, e.g. =main= may have type =(-> ~@ int)=.
...
Indeed, there are some very large and measurable differences between various languages' syntaxes. A study by ETH Zurich of programs in Rosetta Code (a repository of solutions to over 700 programming tasks in hundreds of languages) found that[fn:23]:
