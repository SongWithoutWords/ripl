...
C++ has two built-in kinds of indirection, pointers (adopted from C), and references that were introduced in 1985. Although there are a number of differences between references and pointers in C++, the differences that are relevant to the subject of null safety are:
...
4. Dereferencing a null pointer is undefined behaviour.

Although references in C++ help to mitigate the problems associated with ~null~ because they can be used to express the /intent/ that a value with reference type is not null, they do not provide null-safety, as demonstrated by the program below.


Consequently, although it's not possible to use ~nullptr~ directly in place of a reference in C++, null references can occur (in practice) when a null pointer is dereferenced and used in place of a reference, as demonstrated in the ill-formed program below:
...
// When compiled with GCC 7.3.0 using `gcc -g -std=c++11` and debugged using LLDB,
// the program above produces the following output, and we can see that the address
// of the reference is 0x0, or null:

//   Process 8786 stopped
//     thread #1, name = 'references-not-', stop reason = signal SIGSEGV:
...
Although the C++ Standard addresses this in section 8.3.2,
#+BEGIN_QUOTE
Note: in particular, a null reference cannot exist in a well-defined program, because the only way to create such a reference would be to bind it to the “object” obtained by dereferencing a null pointer, which causes undefined behavior.
#+END_QUOTE
it does so in the typical C++ fashion of declaring it to be undefined behavior. The argument that a reference cannot be null

If C++ cannot even guarantee that a program in which a null pointer is dereferenced will fail, then it seems clear to me that the language cannot be considered null-safe. Although references in C++ may help to mitigate the problems associated with ~null~ because they can be used to express the /intent/ that a value with reference type is not null, they do not actually provide null-safety, as demonstrated above.
