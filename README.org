...
Certain kinds of bugs, problems and anti-patterns (such as null pointer exceptions, memory leaks, and shared or global mutable state), issues that can slow development, block teams, result in unpredictable programs, negatively impact users and cost thousands of dollars, affect only some languages. Just as importantly, and as mentioned by Dijkstra, the ability to express certain thoughts and ideas is contingent on the features of the language in use.

One language that goes a long way towards obviating many of these kinds of problems, in addition to providing many expressive constructs, is Haskell, a language which ([[https://www.cs.utexas.edu/users/EWD/transcriptions/OtherDocs/Haskell.html][to quote Dijkstra again]]) "though not perfect, is of a quality that is several orders of magnitude higher than Java, which is a mess"[fn:5]. Before mentioning some criticisms of Haskell, it's worth mentioning that it feels like a very enlightening language, that operates in many ways at a higher level of abstraction than many other languages (with type classes in particular). Additionally, I find the libraries to be superb.
That said, if Haskell is well-suited for 90% of applications, I do not think that it is well suited to that other 10% of applications that are highly interactive, real time or soft real time, like AAA games.

Before criticising Haskell, I think its worth mentioning that I've found it to be the most 

Unfortunately Haskell solves many of these problems by prohibiting certain useful features, or by placing them behind abstractions (i.e. monads, monad transformers, monad stacks and arrows) that are significantly more complicated than the features or concepts themselves.
