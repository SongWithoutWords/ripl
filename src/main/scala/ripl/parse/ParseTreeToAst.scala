...
case object ParseTreeToAst {
  ...
  def mapExp0(c: rp.Exp0Context): Exp = c match {
    ...
    case c: rp.ApplyContext =>
      App(mapExp0(c.f), mapExps(c.args))

    case c: rp.SelectContext =>
      Select(mapExp0(c.e1), expToNameString(mapExp0(c.e2)))

    case c: rp.UnaryOpContext =>
      App(Name(c.op.getText()), mapExp0(c.e))
  ...
  def mapExp1(c: rp.Exp1Context): Exp = c match {
    ...
    case c: rp.BinOpMulDivModContext =>
      App(Name(c.op.getText()), mapExp1(c.e1), mapExp1(c.e2))
    ...
    case c: rp.BinOpAndContext =>
      App(Name("and"), mapExp1(c.e1), mapExp1(c.e2))

    case c: rp.BinOpOrContext =>
      App(Name("or"), mapExp1(c.e1), mapExp1(c.e2))

    case c: rp.BinOpContext =>
      App(mapExp0(c.op), mapExp1(c.e1), mapExp1(c.e2))

    case c: rp.FunTypeContext =>
      TFun(mapFunParamTypes(c.funTypeParams()), mapExp1(c.exp1()))
    ...
    case c: rp.AssignContext =>
      Assign(mapExp1(c.lhs), mapExp2(c.rhs))
  ...
  def mapDefine(c: rp.DefineContext) = {
    ...
    Define(pair._2, Cons(pair._1, mapExp2(c.rhs)))
  }
  ...
  def mapExp2(c: rp.Exp2Context): Exp = c match {
    case c: rp.IfContext =>
      If(mapExp1(c.e1), mapExp1(c.e2), mapExp2(c.e3))
  ...
  def mapPair(c: rp.PairContext) =
    (mapExp0(c.e1), mapExp0(c.e2))

  def mapPairToNamedExp(c: rp.PairContext): (String, Exp) =
    (expToNameString(mapExp0(c.e2)), mapExp0(c.e1))
