...
case class Float(floatValue: SomeFloat)
case class Null(constantType: Type)
case class Struct(
...
case class Vector(memberValues: List[Constant])
case class Undef(constantType: Type)
case class BlockAddress(blockAddressFunction: Name, blockAddressBlock: Name)
case class GlobalReference(t: Type, name: Name)
case object TokenNone
case class Add(
    nsw: Boolean,
    nuw: Boolean,
    operand0: Constant,
